...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        # required=True,
        default="epoch=99-step=181100.ckpt",
    ...
    kept_preds = all_preds[keep_idx]
    kept_images = all_images[keep_idx]
    kept_labels = all_labels[keep_idx]
    kept_scores = all_scores[keep_idx]

    # Print summary
    n_fg_pred = (kept_preds == 1).sum().item()
    n_fg_gt = (kept_labels == 1).sum().item()
    print(
        f"Predictions: {n_fg_pred} foreground, {len(kept_preds) - n_fg_pred} background"
    )
    print(f"Ground truth: {n_fg_gt} foreground, {len(kept_labels) - n_fg_gt} background")

    # Visualize some sample crops with predictions vs ground truth
    visualize_crops(kept_images, kept_preds, kept_labels, max_examples=args.max_examples)

    # Visualize foreground boxes on the original image
    visualize_foreground_boxes(args.image_id, kept_boxes, kept_preds)
